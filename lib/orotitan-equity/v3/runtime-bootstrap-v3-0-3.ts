...
import runtimeBootstrapJson from "../../../contracts/orotitan-equity/v3/runtime/OROTITAN_RUNTIME_BOOTSTRAP_V3.0.3.json";
import contractPinPackJson from "../../../contracts/orotitan-equity/v3/contract-pin-pack-v3/OROTITAN_CONTRACT_PIN_PACK_V3_0_2.json";

export const OROTITAN_RUNTIME_BOOTSTRAP_VERSION = "3.0.3" as const;
export const OROTITAN_VERSION = "3.1" as const;
...
export type HistoricalParentObservation = {
  ...
  deepDiveContractStatusCode?: string;
  deepDiveBlockerCode?: string;
  deepDiveBlockerClassification?: string;
  deepDiveBlockerScope?: string;
};
...
export type RunContextV3 = {
  ...
  contextVersion: "3.0.3";
  orotitanVersion: "3.1";
  runtimeBootstrapVersion: "3.0.3";
...
export function assertRuntimeBootstrapIntegrity(): void {
  ...
  if (
    ...
    runtimeBootstrap.authority_boundary.methodology_delta !== "VALUATION_DATE_ALIGNMENT_ONLY"
  ) {
  ...
  if (
    activeContractPinPack.contract_pins.process.version !== "3.1" ||
    activeContractPinPack.contract_pins.pilotage.version !== "3.1" ||
    activeContractPinPack.contract_pins.research_stage.version !== "2.0" ||
    activeContractPinPack.contract_pins.deep_dive_stage.version !== "3.1" ||
    activeContractPinPack.contract_pins.integration_stage.version !== "3.1" ||
    activeContractPinPack.contract_pins.dcf_timing.version !== "1.0" ||
    activeContractPinPack.contract_pins.valuation_date_alignment.version !== "1.0"
  ) {
    throw new RuntimeBootstrapError("RUNTIME_AUTHORITY_MISMATCH", "V3 stage authority composition is not exact");
  }

  if (Object.keys(activeContractPinPack.contract_pins).length !== 15) {
    throw new RuntimeBootstrapError("RUNTIME_AUTHORITY_MISMATCH", "V3.0.2 Contract Set must contain exactly 15 logical pins");
  }
}
...
export function buildMethodologyReplaySuccessorPlan(input: {
  ...
  const isExactBlockedRepair =
    input.parent.runStatus === "BLOCKED" &&
    input.parent.contractSetSha256 === "257c287357c19a5d47a42f140a1eb0377d48701b04b07e1e9e740646797c172c" &&
    input.parent.deepDiveLifecycleStatus === "BLOCKED" &&
    input.parent.deepDiveContractStatusCode === "VALUATION_BLOCKED_PINNED_TIMING_DENOMINATOR_DATE_CONFLICT" &&
    input.parent.deepDiveBlockerCode === "V3_VALUATION_TIMING_DENOMINATOR_DATE_CONFLICT" &&
    input.parent.deepDiveBlockerClassification === "PINNED_AUTHORITY_CONFLICT" &&
    input.parent.deepDiveBlockerScope === "VALUATION_DCF_AND_DEPENDENT_OUTPUTS";

  if (input.parent.runStatus !== "ACTIVE" && !isExactBlockedRepair) {
    throw new RuntimeBootstrapError(
      "SUCCESSOR_PARENT_STATUS_MISMATCH",
      "methodology replay requires ACTIVE parent or the exact governed BLOCKED timing-conflict state",
    );
  }
  ...
  if (input.parent.contractSetSha256 === activeContractPinPack.contract_set_sha256) {
    throw new RuntimeBootstrapError("SUCCESSOR_NOT_REQUIRED", "parent is already pinned to the active V3.0.2 Contract Set");
  }
...
export function assertRunContextV3(context: RunContextV3): void {
  ...
  const mismatch = [
    ...
    context.contextVersion !== "3.0.3" && "contextVersion",
