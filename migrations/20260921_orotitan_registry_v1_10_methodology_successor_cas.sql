...
-- Forward-only runtime governance repair. Does not mutate existing runs or create a successor by itself.

begin;

-- Generic run creation remains authoritative for ordinary INITIAL / REFRESH runs.
-- Exact idempotent replays are preserved, but every NEW parent-linked run must use
-- create_orotitan_methodology_successor_run so parent/dossier CAS cannot be bypassed.
...
revoke all on function public.create_orotitan_run(
  text, uuid, text, text, text, date, uuid, uuid, text, text, jsonb, text, text
) from public, anon, authenticated;
grant execute on function public.create_orotitan_run(
  text, uuid, text, text, text, date, uuid, uuid, text, text, jsonb, text, text
) to service_role;
...
comment on function public.create_orotitan_methodology_successor_run(
  text, uuid, text, text, text, date, uuid, uuid, text, text, jsonb, text, text,
  bigint, text, text, text, uuid, uuid
) is 'Controlled same-cutoff methodology replay creation with exact active Contract Set and transactional parent+dossier CAS. SUCCESSOR is lineage; legal run_type remains INITIAL.';
