...
export function TopBar({ onMenuClick, menuButtonRef }: { onMenuClick: () => void; menuButtonRef: RefObject<HTMLButtonElement | null> }) {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-slate-700/60 bg-cockpit-bg/90 backdrop-blur-xl">
      <div className="flex min-h-16 items-center gap-3 px-4 sm:px-6 xl:px-8">
        <button ref={menuButtonRef} type="button" onClick={onMenuClick} className="rounded-control border border-slate-700/60 p-2 text-ink-secondary hover:border-state-accent/40 hover:bg-cockpit-hover hover:text-ink-primary lg:hidden" aria-label="Ouvrir la navigation">
