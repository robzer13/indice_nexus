...
test("expanded local candidate registry preserves ordering while recording Phi-4 mini load fit", () => {
  ...
  assert.equal(registry.status, "POST_C4_PHI4_MINI_LOAD_FIT_RECORDED_INFERENCE_NOT_AUTHORIZED");
  ...
  assert.equal(phi.admission_state, "LOAD_FIT_PASS_CONTEXT4096_INFERENCE_UNTESTED");
