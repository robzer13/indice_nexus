...
test("expanded local candidate registry preserves ordering while recording Phi-4 mini load fit", () => {
  ...
  assert.equal(registry.status, "POST_C4_PHI4_MINI_STMICRO_SEMANTIC_FAIL_FORENSICS_PENDING");
  ...
  assert.equal(phi.admission_state, "C4_STMICRO_SEMANTIC_FAIL_FORENSICS_PENDING_WITH_RELIABILITY_CARRY");
