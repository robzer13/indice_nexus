...
test("RATIONAL forensic V1 records conflict grounding failure then reveals direction overlap", () => {
  const result = JSON.parse(
    readFileSync(
      "calibration/vnext/OROTITAN_GATE18_PHASE_C_C4_RATIONAL_CONFLICT_GROUNDING_FORENSIC_V1_RESULT_001.json",
      "utf8",
    ),
  );
...
test("RATIONAL direction-role forensic V2 is cumulative, read-only, and non-inference", () => {
  const prep = JSON.parse(
    readFileSync(
      "calibration/vnext/OROTITAN_GATE18_PHASE_C_C4_RATIONAL_DIRECTION_ROLE_OVERLAP_FORENSIC_V2_PREP_001.json",
      "utf8",
    ),
  );
  const source = readFileSync(
    "scripts/vnext-gate18-phase-c-c4-rational-direction-role-overlap-forensic-v2.ts",
    "utf8",
  );
  ...
  assert.match(source, /originalDirectionRoleOverlaps/);
  assert.match(source, /removedSupportOverlapRefs/);
  assert.match(source, /retainedRole: "counterevidence"/);
