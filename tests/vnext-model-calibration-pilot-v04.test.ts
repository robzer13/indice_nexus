...
test("Gate 18 v0.4 runner applies one common max output contract", () => {
  ...
  assert.match(source, /model-calibration-pilot-v04/);
  assert.match(
    source,
    /maxOutputTokens:\s*GATE18_PHASE_B_V04_MAX_OUTPUT_TOKENS/,
  );
  assert.match(
    source,
    /GATE18_PHASE_B_V04_MAX_OUTPUT_TOKENS \*\s*modelPricing\.output/,
  );
