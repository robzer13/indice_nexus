...
test("Gate 18 runner advances to current v1.0 while preserving v0.2 packet normalization and paid-call guards", () => {
  ...
  assert.match(
    source,
    /model-calibration-pilot-v10/,
  );
  assert.match(
    source,
    /GATE18_PHASE_B_V10_SCOPE/,
  );
  assert.match(
    source,
    /--allow-multi-model/,
