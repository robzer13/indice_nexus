...
const source = readFileSync(
  "scripts/vnext-gate18-phase-c-c3-adyen-qwen4b-local-run-timeout300.ts",
  "utf8",
);

test("Adyen timeout-only runner changes only the client timeout envelope", () => {
  ...
  assert.doesNotMatch(source, /180_000/);
});

test("Adyen timeout-only runner requires fresh AUTH-002 with exact timeout pin", () => {
  assert.match(
    source,
  ...
  assert.match(
    source,
    /OROTITAN_GATE18_PHASE_C_C3_ADYEN_QWEN3_4B_INFERENCE_AUTH_002\.json/,
  );
  assert.match(source, /client_timeout_ms !== CLIENT_TIMEOUT_MS/);
  assert.match(source, /AUTHORIZED_SINGLE_LOCAL_INFERENCE/);
});

test("Adyen timeout-only runner preserves exact targeted semantic probe", () => {
...
test("Adyen timeout-only runner remains local, private and fail closed", () => {
