...
begin
  ...
  failed := false;
  begin insert into public.research_snapshots select '90000000-0000-4000-8000-000000000003', first_dossier, first_issuer, first_security, 'report-gamma', 'ANALYZE', '2026-09-10', '2026-09-11', 'r1', 'm1', 'c1', 'e1', '04_SCREENER_SCHEMA_V1', '1.0.0', jsonb_set(jsonb_set(base_payload, '{snapshot_id}', '"wrong"'), '{report_id}', '"report-gamma"'), now(); exception when others then get stacked diagnostics actual_constraint_name = CONSTRAINT_NAME; failed := actual_constraint_name = expected_constraint; end;
    if not failed then raise exception 'T15 JSON snapshot_id mismatch was accepted'; end if;
  failed := false;
  begin insert into public.research_snapshots select '90000000-0000-4000-8000-000000000004', first_dossier, first_issuer, first_security, 'report-gamma', 'ANALYZE', '2026-09-10', '2026-09-11', 'r1', 'm1', 'c1', 'e1', '04_SCREENER_SCHEMA_V1', '1.0.0', jsonb_set(jsonb_set(base_payload, '{snapshot_id}', '"90000000-0000-4000-8000-000000000004"'), '{report_id}', '"wrong-report"'), now(); exception when others then get stacked diagnostics actual_constraint_name = CONSTRAINT_NAME; failed := actual_constraint_name = expected_constraint; end;
