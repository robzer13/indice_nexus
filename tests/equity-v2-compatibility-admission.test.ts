...
test("V2.0.10 semantic base uses exact V2.0.10 authority/schema/validator bytes", () => {
  assert.equal(
    sha256("../contracts/orotitan-equity/v2/OROTITAN_CANONICAL_SEMANTIC_COMPATIBILITY_PATCH_V2.0.10.md"),
    V2_COMPATIBILITY_SEMANTIC_BASE.authoritySha256,
  );
  assert.equal(
    sha256("../contracts/orotitan-equity/v2/04_SCREENER_SCHEMA_V1_COMPAT_V2.0.10.json"),
...
test("TEST E valid existing Integration checkpoint routes through V2.0.10-compatible path", () => {
  const input = base();
...
test("TEST I NOT_INTERPRETABLE in unauthorized returnValue field is rejected", () => {
  ...
  input.semanticProjections = [{
    targetField: "l2_research_fundamentals.analytical_metrics.share_count_cagr",
    sourceValue: "NOT_INTERPRETABLE",
    targetValue: "NOT_INTERPRETABLE",
  }];
  const result = evaluateExistingRunCompatibilityAdmission(input);
...
test("all V2.0.2-V2.0.10 field-local semantic rules remain admissible without broadening", () => {
  const rules = [
    ...
    ["l2_research_fundamentals.analytical_metrics.all_in_roic", "NOT_INTERPRETABLE"],
  ] as const;
...
test("V2.0.10 admits exact RD_ADJUSTED_ROIC NOT_INTERPRETABLE for a durable blocked Integration checkpoint", () => {
  const input = base();
  input.runStatus = "BLOCKED";
  input.currentStage = "INTEGRATION";
  input.integrationStage = {
...
test("V2.0.10 still rejects NOT_INTERPRETABLE for unrelated returnValue fields", () => {
  const input = base();
  input.semanticProjections = [{
    targetField: "l2_research_fundamentals.analytical_metrics.share_count_cagr",
    sourceValue: "NOT_INTERPRETABLE",
...
test("V2.0.10 admits exact ALL_IN_ROIC NOT_INTERPRETABLE for a durable paused Integration checkpoint", () => {
  const input = base();
...
test("V2.0.10 rejects arbitrary ALL_IN_ROIC strings", () => {
  for (const value of ["BROKEN", "NOT_INTERPRETABLE_BROKEN", "12.5"]) {
    ...
    const result = evaluateExistingRunCompatibilityAdmission(input);
    assert.equal(result.admitted, false, value);
    if (!result.admitted) assert.ok(result.failures.includes("SEMANTIC_RULE_NOT_AUTHORIZED"), value);
  }
});

test("V2.0.10 keeps NOT_INTERPRETABLE unauthorized for unrelated returnValue fields", () => {
  const input = base();
  input.semanticProjections = [{
    targetField: "l2_research_fundamentals.analytical_metrics.share_count_cagr",
    sourceValue: "NOT_INTERPRETABLE",
