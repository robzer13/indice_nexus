...
export const GATE18_PHASE_B_PROTOCOL_VERSION = "0.8" as const;

export const GATE18_PHASE_B_V08_MODULE_ID =
  GATE18_PHASE_B_V03_MODULE_ID;

export const GATE18_PHASE_B_V08_SCOPE =
  GATE18_PHASE_B_V03_SCOPE;

export const GATE18_PHASE_B_V08_PROMPT_TEMPLATE_ID =
  "GATE18_MOAT_EVIDENCE_AUDIT_V0_6" as const;

export const GATE18_PHASE_B_V08_PROMPT_TEMPLATE_VERSION =
  "0.6" as const;

export const GATE18_PHASE_B_V08_GENERATION_SCHEMA_ID =
  "GATE18_MOAT_EVIDENCE_AUDIT_OUTPUT_V0_6" as const;

export const GATE18_PHASE_B_V08_GENERATION_SCHEMA_VERSION =
  "0.6" as const;

export const GATE18_PHASE_B_V08_MAX_OUTPUT_TOKENS =
  GATE18_PHASE_B_V04_MAX_OUTPUT_TOKENS;

export const GATE18_PHASE_B_V08_SYSTEM_PROMPT = [
...
export const GATE18_PHASE_B_V08_MODULE_QUESTION = [
  ...
  "For every finding, separate directional support from directional counterevidence, then attach optional qualification metadata to any cited evidence whose strength or scope is limited.",
...
export const gate18PhaseBV08OutputSchema = z.object({
...
export type Gate18PhaseBV08Output = z.infer<
  typeof gate18PhaseBV08OutputSchema
>;
...
export const GATE18_PHASE_B_V08_GENERATION_SCHEMA_SPEC = {
  type: "object",
...
function assertCompleteNarrative(
  ...
  if (trimmed.length >= saturationBoundary) {
    throw new Error(
      "VNEXT_GATE18_V08_NARRATIVE_BOUNDARY_SATURATION",
    );
  }
}
...
export function assertGate18PhaseBV08Semantics(
  packet: Gate18V02EvidencePacket,
  output: Gate18PhaseBV08Output,
): void {
  if (output.case_id !== packet.case_id) {
    throw new Error("VNEXT_GATE18_V08_CASE_MISMATCH");
  }
  if (output.data_cutoff !== packet.data_cutoff) {
    throw new Error("VNEXT_GATE18_V08_CUTOFF_MISMATCH");
  }
  ...
  for (const finding of output.priority_findings) {
    assertUnique(
      ...
      "VNEXT_GATE18_V08_DUPLICATE_SUPPORT_REF",
    );
    assertUnique(
      finding.counterevidence_ids,
      "VNEXT_GATE18_V08_DUPLICATE_COUNTEREVIDENCE_REF",
    );
    assertUnique(
      finding.conflict_ids,
      "VNEXT_GATE18_V08_DUPLICATE_FINDING_CONFLICT_REF",
    );
    assertUnique(
    ...
    if (
      ...
      throw new Error(
        "VNEXT_GATE18_V08_COUNTEREVIDENCE_LINK_WITHOUT_IDS",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "VNEXT_GATE18_V08_COUNTEREVIDENCE_LINK_REQUIRED",
      );
    }
    ...
    assertCompleteNarrative(
      finding.claim,
      "VNEXT_GATE18_V08_FINDING_CLAIM_INCOMPLETE",
    );
    assertCompleteNarrative(
      finding.causal_link,
      "VNEXT_GATE18_V08_CAUSAL_LINK_INCOMPLETE",
    );

    for (const qualification of finding.evidence_qualifications) {
      assertCompleteNarrative(
        qualification.qualification,
        "VNEXT_GATE18_V08_EVIDENCE_QUALIFICATION_INCOMPLETE",
      );
    }
    ...
    if (finding.counterevidence_link !== null) {
      assertCompleteNarrative(
        ...
        "VNEXT_GATE18_V08_COUNTEREVIDENCE_LINK_INCOMPLETE",
      );
    }
    ...
    for (const conflictId of finding.conflict_ids) {
      ...
      if (
        ...
        throw new Error(
          "VNEXT_GATE18_V08_CONFLICT_NOT_GROUNDED_IN_FINDING_REFS",
        );
      }
    }
  }
  ...
  assertUnique(
    ...
    "VNEXT_GATE18_V08_DUPLICATE_MATERIAL_CONFLICT",
  );
  ...
  for (const conflict of output.material_conflicts) {
    ...
    assertCompleteNarrative(
      ...
      "VNEXT_GATE18_V08_CONFLICT_IMPLICATION_INCOMPLETE",
    );
  }
  ...
  for (const candidate of output.weak_link_candidates) {
    assertUnique(
      ...
      "VNEXT_GATE18_V08_DUPLICATE_WEAK_LINK_EVIDENCE_REF",
    );
    assertUnique(
      candidate.conflict_ids,
      "VNEXT_GATE18_V08_DUPLICATE_WEAK_LINK_CONFLICT_REF",
    );
    evidenceRefs.push(...candidate.evidence_ids);
    conflictRefs.push(...candidate.conflict_ids);
    assertCompleteNarrative(
      candidate.candidate,
      "VNEXT_GATE18_V08_WEAK_LINK_CANDIDATE_INCOMPLETE",
      138,
    );
    assertCompleteNarrative(
      candidate.why_uncertain,
      "VNEXT_GATE18_V08_WEAK_LINK_EXPLANATION_INCOMPLETE",
    );
  }
  ...
  for (const point of output.unresolved_points) {
    assertUnique(
      point.evidence_ids,
      "VNEXT_GATE18_V08_DUPLICATE_UNRESOLVED_EVIDENCE_REF",
    );
    assertUnique(
      point.conflict_ids,
      "VNEXT_GATE18_V08_DUPLICATE_UNRESOLVED_CONFLICT_REF",
    );
    evidenceRefs.push(...point.evidence_ids);
    conflictRefs.push(...point.conflict_ids);
    assertCompleteNarrative(
      point.question,
      "VNEXT_GATE18_V08_UNRESOLVED_QUESTION_INCOMPLETE",
    );
  }
  ...
  for (const ref of evidenceRefs) {
    if (!/^E-\d{3}$/.test(ref)) {
      throw new Error(
        "VNEXT_GATE18_V08_EVIDENCE_REF_FORMAT_INVALID",
      );
    }
    if (!evidenceIds.has(ref)) {
      throw new Error(
        "VNEXT_GATE18_V08_UNKNOWN_EVIDENCE_REF",
      );
    }
  }
  ...
  for (const ref of conflictRefs) {
    if (!/^C-\d{3}$/.test(ref)) {
      throw new Error(
        "VNEXT_GATE18_V08_CONFLICT_REF_FORMAT_INVALID",
      );
    }
    if (!conflictsById.has(ref)) {
      throw new Error(
        "VNEXT_GATE18_V08_UNKNOWN_CONFLICT_REF",
      );
    }
  }
}

export function buildVerifiedGate18V08MoatPacket(
  ...args: Parameters<typeof buildVerifiedGate18V03MoatPacket>
): Gate18V02VerifiedPacket {
  return buildVerifiedGate18V03MoatPacket(...args);
}

export function buildGate18PhaseBV08ModelInput(
  packet: Gate18V02EvidencePacket,
): string {
  ...
  if (packetStart < 0) {
    throw new Error(
      "VNEXT_GATE18_V08_PACKET_MARKER_MISSING",
    );
  }

  return [
    GATE18_PHASE_B_V08_MODULE_QUESTION,
    "",
    v03.slice(packetStart),
  ].join("\n");
}

export function gate18PhaseBV08PromptTemplateSha256(): string {
  return sha256Hex(
    JSON.stringify({
      system: GATE18_PHASE_B_V08_SYSTEM_PROMPT,
      question: GATE18_PHASE_B_V08_MODULE_QUESTION,
    }),
  );
}

export function gate18PhaseBV08GenerationSchemaSha256(): string {
  return sha256Hex(
    JSON.stringify(
      GATE18_PHASE_B_V08_GENERATION_SCHEMA_SPEC,
    ),
  );
}
