...
export const GATE18_PHASE_B_PROTOCOL_VERSION = "0.7" as const;

export const GATE18_PHASE_B_V07_MODULE_ID =
  GATE18_PHASE_B_V03_MODULE_ID;

export const GATE18_PHASE_B_V07_SCOPE =
  GATE18_PHASE_B_V03_SCOPE;

export const GATE18_PHASE_B_V07_PROMPT_TEMPLATE_ID =
  "GATE18_MOAT_EVIDENCE_AUDIT_V0_5" as const;

export const GATE18_PHASE_B_V07_PROMPT_TEMPLATE_VERSION =
  "0.5" as const;

export const GATE18_PHASE_B_V07_GENERATION_SCHEMA_ID =
  "GATE18_MOAT_EVIDENCE_AUDIT_OUTPUT_V0_5" as const;

export const GATE18_PHASE_B_V07_GENERATION_SCHEMA_VERSION =
  "0.5" as const;

export const GATE18_PHASE_B_V07_MAX_OUTPUT_TOKENS =
  GATE18_PHASE_B_V04_MAX_OUTPUT_TOKENS;

export const GATE18_PHASE_B_V07_SYSTEM_PROMPT = [
  ...
  "For each priority finding, evidence_ids contains only evidence that directly supports the wording of that claim.",
  "qualification_evidence_ids contains evidence that supports or contextualizes the claim but limits its strength, representativeness, generalizability, measurement quality, or validation. Qualification is not opposition.",
  "counterevidence_ids contains only evidence that directly weakens or contradicts the wording of that claim through an opposing mechanism, contrary observation, displacement, substitution, or economically inconsistent outcome.",
  "Judge all three roles relative to the exact claim wording. For a skeptical claim, substitution or weak lock-in may support the claim, while low churn or deep integration may be counterevidence.",
  "Issuer-hosted, selected, unaudited, non-representative, or denominator-limited evidence usually belongs in qualification_evidence_ids when it still supports the bounded claim.",
  ...
  "Never place the same evidence ID in more than one of evidence_ids, qualification_evidence_ids, and counterevidence_ids for one finding.",
  "If qualification_evidence_ids is non-empty, qualification_link must explain in one complete sentence how those IDs limit the strength or scope of the claim without contradicting it. If empty, qualification_link must be null.",
...
export const GATE18_PHASE_B_V07_MODULE_QUESTION = [
  ...
  "For every finding, separate supporting evidence, qualification evidence that limits strength without opposition, and counterevidence that weakens the exact claim.",
...
export const gate18PhaseBV07OutputSchema = z.object({
  ...
  priority_findings: z
    .array(
      z.object({
        ...
        qualification_evidence_ids: z.array(evidenceIdSchema).max(2),
        qualification_link: z
          .string()
          .min(1)
          .max(180)
          .nullable(),
...
export type Gate18PhaseBV07Output = z.infer<
  typeof gate18PhaseBV07OutputSchema
>;
...
export const GATE18_PHASE_B_V07_GENERATION_SCHEMA_SPEC = {
  type: "object",
...
function assertCompleteNarrative(
  ...
  if (trimmed.length >= saturationBoundary) {
    throw new Error(
      "VNEXT_GATE18_V07_NARRATIVE_BOUNDARY_SATURATION",
    );
  }
}
...
export function assertGate18PhaseBV07Semantics(
  packet: Gate18V02EvidencePacket,
  output: Gate18PhaseBV07Output,
): void {
  if (output.case_id !== packet.case_id) {
    throw new Error("VNEXT_GATE18_V07_CASE_MISMATCH");
  }
  if (output.data_cutoff !== packet.data_cutoff) {
    throw new Error("VNEXT_GATE18_V07_CUTOFF_MISMATCH");
  }
  ...
  for (const finding of output.priority_findings) {
    assertUnique(
      ...
      "VNEXT_GATE18_V07_DUPLICATE_SUPPORT_REF",
    );
    assertUnique(
      finding.qualification_evidence_ids,
      "VNEXT_GATE18_V07_DUPLICATE_QUALIFICATION_REF",
    );
    assertUnique(
      finding.counterevidence_ids,
      "VNEXT_GATE18_V07_DUPLICATE_COUNTEREVIDENCE_REF",
    );
    assertUnique(
      finding.conflict_ids,
      "VNEXT_GATE18_V07_DUPLICATE_FINDING_CONFLICT_REF",
    );
    ...
    if (
      finding.qualification_evidence_ids.some((id) =>
        supportIds.has(id),
      )
    ) {
      throw new Error(
    ...
    if (
      finding.counterevidence_ids.some((id) =>
        supportIds.has(id) || qualificationIds.has(id),
      )
    ) {
      throw new Error(
    ...
    if (
      finding.qualification_evidence_ids.length === 0 &&
      finding.qualification_link !== null
    ) {
      throw new Error(
    ...
    if (
      ...
      throw new Error(
        "VNEXT_GATE18_V07_COUNTEREVIDENCE_LINK_WITHOUT_IDS",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "VNEXT_GATE18_V07_COUNTEREVIDENCE_LINK_REQUIRED",
      );
    }
    ...
    assertCompleteNarrative(
      finding.claim,
      "VNEXT_GATE18_V07_FINDING_CLAIM_INCOMPLETE",
    );
    assertCompleteNarrative(
      finding.causal_link,
      "VNEXT_GATE18_V07_CAUSAL_LINK_INCOMPLETE",
    );

    if (finding.qualification_link !== null) {
      assertCompleteNarrative(
        finding.qualification_link,
        "VNEXT_GATE18_V07_QUALIFICATION_LINK_INCOMPLETE",
      );
    }
    ...
    if (finding.counterevidence_link !== null) {
      assertCompleteNarrative(
        ...
        "VNEXT_GATE18_V07_COUNTEREVIDENCE_LINK_INCOMPLETE",
      );
    }
  ...
  assertUnique(
    ...
    "VNEXT_GATE18_V07_DUPLICATE_MATERIAL_CONFLICT",
  );
  ...
  for (const conflict of output.material_conflicts) {
    ...
    assertCompleteNarrative(
      ...
      "VNEXT_GATE18_V07_CONFLICT_IMPLICATION_INCOMPLETE",
    );
  }
  ...
  for (const candidate of output.weak_link_candidates) {
    assertUnique(
      ...
      "VNEXT_GATE18_V07_DUPLICATE_WEAK_LINK_EVIDENCE_REF",
    );
    assertUnique(
      candidate.conflict_ids,
      "VNEXT_GATE18_V07_DUPLICATE_WEAK_LINK_CONFLICT_REF",
    );
    evidenceRefs.push(...candidate.evidence_ids);
    conflictRefs.push(...candidate.conflict_ids);
    assertCompleteNarrative(
      candidate.candidate,
      "VNEXT_GATE18_V07_WEAK_LINK_CANDIDATE_INCOMPLETE",
      138,
    );
    assertCompleteNarrative(
      candidate.why_uncertain,
      "VNEXT_GATE18_V07_WEAK_LINK_EXPLANATION_INCOMPLETE",
    );
  }
  ...
  for (const point of output.unresolved_points) {
    assertUnique(
      point.evidence_ids,
      "VNEXT_GATE18_V07_DUPLICATE_UNRESOLVED_EVIDENCE_REF",
    );
    assertUnique(
      point.conflict_ids,
      "VNEXT_GATE18_V07_DUPLICATE_UNRESOLVED_CONFLICT_REF",
    );
    evidenceRefs.push(...point.evidence_ids);
    conflictRefs.push(...point.conflict_ids);
    assertCompleteNarrative(
      point.question,
      "VNEXT_GATE18_V07_UNRESOLVED_QUESTION_INCOMPLETE",
    );
  }
  ...
  for (const ref of evidenceRefs) {
    if (!/^E-\d{3}$/.test(ref)) {
      throw new Error(
        "VNEXT_GATE18_V07_EVIDENCE_REF_FORMAT_INVALID",
      );
    }
    if (!evidenceIds.has(ref)) {
      throw new Error(
        "VNEXT_GATE18_V07_UNKNOWN_EVIDENCE_REF",
      );
    }
  }
  ...
  for (const ref of conflictRefs) {
    if (!/^C-\d{3}$/.test(ref)) {
      throw new Error(
        "VNEXT_GATE18_V07_CONFLICT_REF_FORMAT_INVALID",
      );
    }
    if (!conflictsById.has(ref)) {
      throw new Error(
        "VNEXT_GATE18_V07_UNKNOWN_CONFLICT_REF",
      );
    }
  }
}

export function buildVerifiedGate18V07MoatPacket(
  ...args: Parameters<typeof buildVerifiedGate18V03MoatPacket>
): Gate18V02VerifiedPacket {
  return buildVerifiedGate18V03MoatPacket(...args);
}

export function buildGate18PhaseBV07ModelInput(
  packet: Gate18V02EvidencePacket,
): string {
  ...
  if (packetStart < 0) {
    throw new Error(
      "VNEXT_GATE18_V07_PACKET_MARKER_MISSING",
    );
  }

  return [
    GATE18_PHASE_B_V07_MODULE_QUESTION,
    "",
    v03.slice(packetStart),
  ].join("\n");
}

export function gate18PhaseBV07PromptTemplateSha256(): string {
  return sha256Hex(
    JSON.stringify({
      system: GATE18_PHASE_B_V07_SYSTEM_PROMPT,
      question: GATE18_PHASE_B_V07_MODULE_QUESTION,
    }),
  );
}

export function gate18PhaseBV07GenerationSchemaSha256(): string {
  return sha256Hex(
    JSON.stringify(
      GATE18_PHASE_B_V07_GENERATION_SCHEMA_SPEC,
    ),
  );
}
