...
export const GATE18_PHASE_B_PROTOCOL_VERSION = "0.6" as const;

export const GATE18_PHASE_B_V06_MODULE_ID =
  GATE18_PHASE_B_V03_MODULE_ID;

export const GATE18_PHASE_B_V06_SCOPE =
  GATE18_PHASE_B_V03_SCOPE;

export const GATE18_PHASE_B_V06_PROMPT_TEMPLATE_ID =
  "GATE18_MOAT_EVIDENCE_AUDIT_V0_4" as const;

export const GATE18_PHASE_B_V06_PROMPT_TEMPLATE_VERSION =
  "0.4" as const;

export const GATE18_PHASE_B_V06_GENERATION_SCHEMA_ID =
  "GATE18_MOAT_EVIDENCE_AUDIT_OUTPUT_V0_4" as const;

export const GATE18_PHASE_B_V06_GENERATION_SCHEMA_VERSION =
  "0.4" as const;

export const GATE18_PHASE_B_V06_MAX_OUTPUT_TOKENS =
  GATE18_PHASE_B_V04_MAX_OUTPUT_TOKENS;

export const GATE18_PHASE_B_V06_SYSTEM_PROMPT = [
  ...
  "counterevidence_ids contains only evidence that directly weakens, qualifies, or contradicts the wording of that claim.",
  "Judge evidence polarity relative to the exact claim wording. For a skeptical claim, evidence of substitution or weak lock-in supports the claim, while evidence of low churn or deep integration may be counterevidence.",
  "A limitation on supportive evidence is not automatically counterevidence. Evidence that reinforces the stated limitation remains support for the bounded claim.",
...
export const GATE18_PHASE_B_V06_MODULE_QUESTION = [
  ...
  "For every finding, separate supporting evidence from evidence that weakens that exact claim.",
...
export const gate18PhaseBV06OutputSchema = z.object({
...
export type Gate18PhaseBV06Output = z.infer<
  typeof gate18PhaseBV06OutputSchema
>;
...
export const GATE18_PHASE_B_V06_GENERATION_SCHEMA_SPEC = {
  type: "object",
...
function assertCompleteNarrative(
  ...
  if (trimmed.length >= saturationBoundary) {
    throw new Error(
      "VNEXT_GATE18_V06_NARRATIVE_BOUNDARY_SATURATION",
    );
  }
}
...
export function assertGate18PhaseBV06Semantics(
  packet: Gate18V02EvidencePacket,
  output: Gate18PhaseBV06Output,
): void {
  if (output.case_id !== packet.case_id) {
    throw new Error("VNEXT_GATE18_V06_CASE_MISMATCH");
  }
  if (output.data_cutoff !== packet.data_cutoff) {
    throw new Error("VNEXT_GATE18_V06_CUTOFF_MISMATCH");
  }
  ...
  for (const finding of output.priority_findings) {
    assertUnique(
      ...
      "VNEXT_GATE18_V06_DUPLICATE_SUPPORT_REF",
    );
    assertUnique(
      finding.counterevidence_ids,
      "VNEXT_GATE18_V06_DUPLICATE_COUNTEREVIDENCE_REF",
    );
    assertUnique(
      finding.conflict_ids,
      "VNEXT_GATE18_V06_DUPLICATE_FINDING_CONFLICT_REF",
    );
    ...
    assertCompleteNarrative(
      finding.claim,
      "VNEXT_GATE18_V06_FINDING_CLAIM_INCOMPLETE",
    );
    assertCompleteNarrative(
      finding.causal_link,
      "VNEXT_GATE18_V06_CAUSAL_LINK_INCOMPLETE",
    );

    if (finding.counterevidence_link !== null) {
      assertCompleteNarrative(
        finding.counterevidence_link,
        "VNEXT_GATE18_V06_COUNTEREVIDENCE_LINK_INCOMPLETE",
      );
    }
  ...
  assertUnique(
    ...
    "VNEXT_GATE18_V06_DUPLICATE_MATERIAL_CONFLICT",
  );
  ...
  for (const conflict of output.material_conflicts) {
    ...
    assertCompleteNarrative(
      ...
      "VNEXT_GATE18_V06_CONFLICT_IMPLICATION_INCOMPLETE",
    );
  }
  ...
  for (const candidate of output.weak_link_candidates) {
    assertUnique(
      ...
      "VNEXT_GATE18_V06_DUPLICATE_WEAK_LINK_EVIDENCE_REF",
    );
    assertUnique(
      candidate.conflict_ids,
      "VNEXT_GATE18_V06_DUPLICATE_WEAK_LINK_CONFLICT_REF",
    );
    evidenceRefs.push(...candidate.evidence_ids);
    conflictRefs.push(...candidate.conflict_ids);
    assertCompleteNarrative(
      candidate.candidate,
      "VNEXT_GATE18_V06_WEAK_LINK_CANDIDATE_INCOMPLETE",
      138,
    );
    assertCompleteNarrative(
      candidate.why_uncertain,
      "VNEXT_GATE18_V06_WEAK_LINK_EXPLANATION_INCOMPLETE",
    );
  }
  ...
  for (const point of output.unresolved_points) {
    assertUnique(
      point.evidence_ids,
      "VNEXT_GATE18_V06_DUPLICATE_UNRESOLVED_EVIDENCE_REF",
    );
    assertUnique(
      point.conflict_ids,
      "VNEXT_GATE18_V06_DUPLICATE_UNRESOLVED_CONFLICT_REF",
    );
    evidenceRefs.push(...point.evidence_ids);
    conflictRefs.push(...point.conflict_ids);
    assertCompleteNarrative(
      point.question,
      "VNEXT_GATE18_V06_UNRESOLVED_QUESTION_INCOMPLETE",
    );
  }
  ...
  for (const ref of evidenceRefs) {
    if (!/^E-\d{3}$/.test(ref)) {
      throw new Error(
        "VNEXT_GATE18_V06_EVIDENCE_REF_FORMAT_INVALID",
      );
    }
    if (!evidenceIds.has(ref)) {
      throw new Error(
        "VNEXT_GATE18_V06_UNKNOWN_EVIDENCE_REF",
      );
    }
  }
  ...
  for (const ref of conflictRefs) {
    if (!/^C-\d{3}$/.test(ref)) {
      throw new Error(
        "VNEXT_GATE18_V06_CONFLICT_REF_FORMAT_INVALID",
      );
    }
    if (!conflictsById.has(ref)) {
      throw new Error(
        "VNEXT_GATE18_V06_UNKNOWN_CONFLICT_REF",
      );
    }
  }
}

export function buildVerifiedGate18V06MoatPacket(
  ...args: Parameters<typeof buildVerifiedGate18V03MoatPacket>
): Gate18V02VerifiedPacket {
  return buildVerifiedGate18V03MoatPacket(...args);
}

export function buildGate18PhaseBV06ModelInput(
  packet: Gate18V02EvidencePacket,
): string {
  ...
  if (packetStart < 0) {
    throw new Error(
      "VNEXT_GATE18_V06_PACKET_MARKER_MISSING",
    );
  }

  return [
    GATE18_PHASE_B_V06_MODULE_QUESTION,
    "",
    v03.slice(packetStart),
  ].join("\n");
}

export function gate18PhaseBV06PromptTemplateSha256(): string {
  return sha256Hex(
    JSON.stringify({
      system: GATE18_PHASE_B_V06_SYSTEM_PROMPT,
      question: GATE18_PHASE_B_V06_MODULE_QUESTION,
    }),
  );
}

export function gate18PhaseBV06GenerationSchemaSha256(): string {
  return sha256Hex(
    JSON.stringify(
      GATE18_PHASE_B_V06_GENERATION_SCHEMA_SPEC,
    ),
  );
}
