...
export const GATE18_PHASE_B_PROTOCOL_VERSION = "1.0" as const;

export const GATE18_PHASE_B_V10_MODULE_ID =
  GATE18_PHASE_B_V03_MODULE_ID;

export const GATE18_PHASE_B_V10_SCOPE =
  GATE18_PHASE_B_V03_SCOPE;

export const GATE18_PHASE_B_V10_PROMPT_TEMPLATE_ID =
  "GATE18_MOAT_EVIDENCE_AUDIT_V0_8" as const;

export const GATE18_PHASE_B_V10_PROMPT_TEMPLATE_VERSION =
  "0.8" as const;

export const GATE18_PHASE_B_V10_GENERATION_SCHEMA_ID =
  "GATE18_MOAT_EVIDENCE_AUDIT_OUTPUT_V0_6" as const;

export const GATE18_PHASE_B_V10_GENERATION_SCHEMA_VERSION =
  "0.6" as const;

export const GATE18_PHASE_B_V10_MAX_OUTPUT_TOKENS =
  GATE18_PHASE_B_V04_MAX_OUTPUT_TOKENS;

export const GATE18_PHASE_B_V10_SYSTEM_PROMPT = [
  ...
  "Before assigning any counterevidence ID, apply this counterfactual test: if that evidence is true, does the exact claim become materially less likely, less strong, or less economically valid? If not, it is not counterevidence to that claim.",
  "Evidence that can be fully true at the same time as the exact claim is not counterevidence merely because it challenges a downstream inference, a broader moat mechanism, or a different scope, period, or event.",
  "When the packet contains opposition to an economic inference but not to a raw descriptive observation, either write the finding at the economically contestable inference level or leave counterevidence_ids empty for the descriptive claim.",
  "For a MIXED finding, support and counterevidence must address the same semantic target. Do not make an existence claim MIXED using evidence that only changes frequency, scope, severity, persistence, or economic interpretation.",
  "A conflict resolved by scope must not create MIXED status unless the residual opposing evidence still weakens the exact claim after the scope distinction is respected.",
...
export const GATE18_PHASE_B_V10_MODULE_QUESTION = [
  ...
  "For every finding, write one atomic directional claim at the semantic level where both support and any counterevidence genuinely bear on the same proposition; apply the counterfactual counterevidence test before assigning opposition.",
  "Then separate directional support from directional counterevidence and attach optional qualification metadata to cited evidence whose strength or scope is limited.",
...
export const gate18PhaseBV10OutputSchema = z.object({
...
export type Gate18PhaseBV10Output = z.infer<
  typeof gate18PhaseBV10OutputSchema
>;
...
export const GATE18_PHASE_B_V10_GENERATION_SCHEMA_SPEC = {
  type: "object",
...
export function assertGate18PhaseBV10Semantics(
  packet: Gate18V02EvidencePacket,
  output: Gate18PhaseBV10Output,
): void {
  if (output.case_id !== packet.case_id) {
    throw new Error("VNEXT_GATE18_V10_CASE_MISMATCH");
  }
  if (output.data_cutoff !== packet.data_cutoff) {
    throw new Error("VNEXT_GATE18_V10_CUTOFF_MISMATCH");
  }
  ...
  for (const finding of output.priority_findings) {
    assertUnique(
      ...
      "VNEXT_GATE18_V10_DUPLICATE_SUPPORT_REF",
    );
    assertUnique(
      finding.counterevidence_ids,
      "VNEXT_GATE18_V10_DUPLICATE_COUNTEREVIDENCE_REF",
    );
    assertUnique(
      finding.conflict_ids,
      "VNEXT_GATE18_V10_DUPLICATE_FINDING_CONFLICT_REF",
    );
    assertUnique(
      finding.evidence_qualifications.map(
        (item) => item.evidence_id,
      ),
      "VNEXT_GATE18_V10_DUPLICATE_QUALIFICATION_REF",
    );
    ...
    for (const qualification of finding.evidence_qualifications) {
      if (
        ...
        throw new Error(
          "VNEXT_GATE18_V10_ORPHAN_EVIDENCE_QUALIFICATION",
        );
      }
    }
    ...
    if (
      ...
      throw new Error(
        "VNEXT_GATE18_V10_COUNTEREVIDENCE_LINK_REQUIRED",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "VNEXT_GATE18_V10_MIXED_REQUIRES_CHALLENGE",
      );
    }
    ...
    for (const qualification of finding.evidence_qualifications) {
      assertCompleteNarrative(
        ...
        "VNEXT_GATE18_V10_EVIDENCE_QUALIFICATION_INCOMPLETE",
      );
    }
  ...
  assertUnique(
    ...
    "VNEXT_GATE18_V10_DUPLICATE_MATERIAL_CONFLICT",
  );
  ...
  for (const candidate of output.weak_link_candidates) {
    assertUnique(
      ...
      "VNEXT_GATE18_V10_DUPLICATE_WEAK_LINK_EVIDENCE_REF",
    );
    assertUnique(
      candidate.conflict_ids,
      "VNEXT_GATE18_V10_DUPLICATE_WEAK_LINK_CONFLICT_REF",
    );
  ...
  for (const point of output.unresolved_points) {
    assertUnique(
      point.evidence_ids,
      "VNEXT_GATE18_V10_DUPLICATE_UNRESOLVED_EVIDENCE_REF",
    );
    assertUnique(
      point.conflict_ids,
      "VNEXT_GATE18_V10_DUPLICATE_UNRESOLVED_CONFLICT_REF",
    );
    evidenceRefs.push(...point.evidence_ids);
    conflictRefs.push(...point.conflict_ids);
    assertCompleteNarrative(
      point.question,
      "VNEXT_GATE18_V10_UNRESOLVED_QUESTION_INCOMPLETE",
    );
  }
  ...
  for (const ref of evidenceRefs) {
    if (!/^E-\d{3}$/.test(ref)) {
      throw new Error(
        "VNEXT_GATE18_V10_EVIDENCE_REF_FORMAT_INVALID",
      );
    }
    if (!evidenceIds.has(ref)) {
      throw new Error(
        "VNEXT_GATE18_V10_UNKNOWN_EVIDENCE_REF",
      );
    }
  }
  ...
  for (const ref of conflictRefs) {
    if (!/^C-\d{3}$/.test(ref)) {
      throw new Error(
        "VNEXT_GATE18_V10_CONFLICT_REF_FORMAT_INVALID",
      );
    }
...
export function buildGate18PhaseBV10ModelInput(
  ...
  if (packetStart < 0) {
    throw new Error(
      "VNEXT_GATE18_V10_PACKET_MARKER_MISSING",
    );
  }

  return [
    GATE18_PHASE_B_V10_MODULE_QUESTION,
    "",
    v03.slice(packetStart),
  ].join("\n");
}

export function gate18PhaseBV10PromptTemplateSha256(): string {
  return sha256Hex(
    JSON.stringify({
      system: GATE18_PHASE_B_V10_SYSTEM_PROMPT,
      question: GATE18_PHASE_B_V10_MODULE_QUESTION,
    }),
  );
}

export function gate18PhaseBV10GenerationSchemaSha256(): string {
  return sha256Hex(
    JSON.stringify(
