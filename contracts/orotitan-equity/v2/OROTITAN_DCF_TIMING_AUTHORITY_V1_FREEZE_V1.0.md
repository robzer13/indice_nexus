...
```text
analysis standard -> economic DCF framework and basis matching
V2 process -> stage sequencing, point-in-time and immutable run pins
V2 pilotage -> exact run routing
V2 Deep Dive -> Valuation consumes frozen valuation policy/conventions
integration -> projects certified outputs, never re-values
I2 / I3-B -> unchanged
```
...
A controlled successor run is permitted only to repair a run whose output depends on this newly frozen convention.

`SUCCESSOR` is lineage, not a legal `RUN_TYPE`.

For an unpublished parent that has no current canonical snapshot:
...
CONTRACT_SET = newly frozen Contract Set
```

For a parent with a valid canonical current snapshot, ordinary frozen REFRESH rules govern instead.

Same-cutoff replay rules:

```text
PARENT RUN = immutable
PARENT CONTRACT SET = immutable
PARENT VALUATION OUTPUTS = historical only, not successor analytical authority
RESEARCH / FUNDAMENTALS inputs may cross runs only through explicit hash-verified lineage and REVALIDATES / DERIVED_FROM edges
VALUATION outputs and VALUATION_LOCK from the parent may not be revalidated across this methodology boundary
FIRST ANALYTICAL PHASE = VALUATION only after exact upstream non-valuation inputs are revalidated into the successor
NO NEW POST-CUTOFF EVIDENCE
NO NEW FUNDAMENTAL JUDGMENT
NO CALIBRATION TO PARENT VALUATION
CERTIFICATION / INTEGRATION remain downstream work and are not executed by the governance-repair workstream
```

If the upstream non-valuation artifacts cannot be revalidated without analytical change, the successor must route to the earliest affected phase rather than force a Valuation-only replay.
...
No historical Valuation Lock is deleted or rewritten. No new Valuation Lock is created by the governance-repair workstream.
