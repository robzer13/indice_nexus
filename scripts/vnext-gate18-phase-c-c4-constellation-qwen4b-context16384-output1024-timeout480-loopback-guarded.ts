...
const CLIENT_TIMEOUT_MS = 480_000;
const AUTHORIZATION_ID =
  "G18-PHASEC-C4-CONSTELLATION-QWEN4B-CONTEXT16384-OUTPUT1024-TIMEOUT480-LOOPBACK-AUTH-001";
const AUTHORIZATION_PATH =
  "calibration/vnext/OROTITAN_GATE18_PHASE_C_C4_CONSTELLATION_QWEN3_4B_CONTEXT16384_OUTPUT1024_TIMEOUT480_LOOPBACK_AUTH_001.json";
...
const EXPECTED_PROMPT_BYTES = 9041;
const EXPECTED_EVIDENCE_COUNT = 11;
const EXPECTED_CONFLICT_COUNT = 1;
...
function findConstellationSoftware(): Gate18PilotCompany {
  ...
  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_C4_LOCAL_CONSTELLATION_NOT_UNIQUE",
    );
  }
...
function readAndAssertAuthorization(
  ...
  if (
    ...
    artifact.status !== "AUTHORIZED_SINGLE_LOCAL_C4_TIMEOUT_REMEDIATION" ||
    ...
    artifact.c4_inference.company !== "Constellation Software" ||
...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(
    privateDir,
    `${isoFileSafe(completedAt)}__C4_CONSTELLATION_MOAT_EVIDENCE_AUDIT_QWEN4B_CONTEXT16384_001.json`,
  );
