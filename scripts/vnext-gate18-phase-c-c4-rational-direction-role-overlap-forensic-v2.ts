...
function parseArgs(argv: readonly string[]): {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    throw new Error(
      `VNEXT_GATE18_C4_RATIONAL_DIRECTION_OVERLAP_FORENSIC_V2_UNKNOWN_ARG:${argv[i]}`,
    );
  }

  if (!privateRepoRoot.trim()) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_DIRECTION_OVERLAP_FORENSIC_V2_PRIVATE_REPO_ROOT_REQUIRED",
    );
  }
  if (!privateArtifact.trim()) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_DIRECTION_OVERLAP_FORENSIC_V2_PRIVATE_ARTIFACT_REQUIRED",
    );
  }
...
function findRational(): Gate18PilotCompany {
  ...
  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_DIRECTION_OVERLAP_FORENSIC_V2_COMPANY_NOT_UNIQUE",
    );
  }
...
function artifactReader(
  ...
  return (pin) => {
    ...
    if (!normalizedAbsolute.startsWith(prefix)) {
      throw new Error(
        "VNEXT_GATE18_C4_RATIONAL_DIRECTION_OVERLAP_FORENSIC_V2_PRIVATE_PATH_ESCAPE",
      );
    }
...
function main(): void {
  ...
  if (
    ...
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_DIRECTION_OVERLAP_FORENSIC_V2_RUN_IDENTITY_MISMATCH",
    );
  }
  ...
  if (!parsed.success) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_DIRECTION_OVERLAP_FORENSIC_V2_SCHEMA_REPARSE_FAILED",
    );
  }
  ...
  if (verified.packetSha256 !== EXPECTED_PACKET_SHA256) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_DIRECTION_OVERLAP_FORENSIC_V2_PACKET_IDENTITY_MISMATCH",
    );
  }
  ...
  console.log(
    JSON.stringify(
      {
        format:
          "OROTITAN_GATE18_PHASE_C_C4_RATIONAL_DIRECTION_ROLE_OVERLAP_FORENSIC_V0.1",
        gate: 18,
        ...
        interpretationBoundary: {
          ...
          purpose:
            "Reveal whether any downstream deterministic semantic defect remains after cumulative diagnostic normalization of finding-conflict grounding and direction-role overlap.",
        },
        safety: {
