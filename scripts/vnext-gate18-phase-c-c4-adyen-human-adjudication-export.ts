...
function parseArgs(argv: readonly string[]): CliOptions {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    throw new Error(
      `VNEXT_GATE18_C4_ADYEN_ADJUDICATION_EXPORT_UNKNOWN_ARG:${arg}`,
    );
  }

  if (privateRepoRoot.trim().length === 0) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_ADJUDICATION_EXPORT_PRIVATE_REPO_REQUIRED",
    );
  }

  if (privateOutputPath.trim().length === 0) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_ADJUDICATION_EXPORT_PRIVATE_RUN_REQUIRED",
    );
  }
...
function findAdyen(): Gate18PilotCompany {
  ...
  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_ADJUDICATION_EXPORT_COMPANY_NOT_UNIQUE",
    );
  }
...
function artifactReader(
  ...
  return (pin) => {
    ...
    if (!normalizedAbsolute.startsWith(normalizedPrefix)) {
      throw new Error(
        "VNEXT_GATE18_C4_ADYEN_ADJUDICATION_EXPORT_PRIVATE_PATH_ESCAPE",
      );
    }
...
function main(): void {
  ...
  if (!existsSync(privateOutputAbsolute)) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_ADJUDICATION_EXPORT_PRIVATE_RUN_NOT_FOUND",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_ADJUDICATION_EXPORT_PRIVATE_RUN_IDENTITY_OR_ENGINEERING_FAIL_MISMATCH",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_ADJUDICATION_EXPORT_REBUILT_PACKET_IDENTITY_MISMATCH",
    );
  }
  ...
  const reviewPacket = {
    format:
      "OROTITAN_GATE18_PHASE_C_C4_ADYEN_HUMAN_ADJUDICATION_PACKET_V0.1",
  ...
  console.log(
    JSON.stringify(
      {
        format:
          "OROTITAN_GATE18_PHASE_C_C4_ADYEN_HUMAN_ADJUDICATION_EXPORT_SUMMARY_V0.1",
