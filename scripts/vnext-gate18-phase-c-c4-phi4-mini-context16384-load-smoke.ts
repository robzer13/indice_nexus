...
const AUTHORIZATION_ID = "G18-PHASEC-C4-PHI4-MINI-CONTEXT16384-LOAD-SMOKE-AUTH-001";
...
function requireAuthorization(): void {
  if (!existsSync(AUTH_PATH)) {
    throw new Error("PHI4_MINI_CONTEXT16384_LOAD_SMOKE_AUTHORIZATION_ARTIFACT_MISSING");
  }
  ...
  if (
    ...
    authority.context16384_load_smoke_authorized !== true ||
    authority.context16384_inference_authorized !== false
  ) {
    throw new Error("PHI4_MINI_CONTEXT16384_LOAD_SMOKE_AUTHORIZATION_NOT_ACTIVE");
  }
}
...
async function main(): Promise<void> {
  ...
  if (!loadOnlyConfirmed) {
    throw new Error("PHI4_MINI_CONTEXT16384_LOAD_ONLY_GUARD_FAILED");
  }
  ...
  if (!unloadConfirmed) {
    throw new Error("PHI4_MINI_CONTEXT16384_UNLOAD_GUARD_FAILED");
  }
  ...
  const payload = {
    format: "OROTITAN_GATE18_PHASE_C_C4_PHI4_MINI_CONTEXT16384_LOAD_SMOKE_V0.1",
    ...
    interpretationBoundary: {
      ...
      context16384InferenceAuthorized: false,
      c4InferenceAuthorized: false,
      nextAction:
        "Persist context16384 load-only measurements and review hardware fit before any C4 inference execution.",
    },
  };
