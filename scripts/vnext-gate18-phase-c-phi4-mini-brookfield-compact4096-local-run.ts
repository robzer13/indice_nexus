...
const AUTHORIZATION_ID =
  "G18-PHASEC-PHI4-MINI-BROOKFIELD-COMPACT4096-INFERENCE-AUTH-001";
...
function parseArgs(argv: readonly string[]): CliOptions {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    throw new Error(
      `VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_UNKNOWN_ARG:${arg}`,
    );
  }

  if (privateRepoRoot.trim().length === 0) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_PRIVATE_REPO_ROOT_REQUIRED",
    );
  }
...
function findBrookfield(): Gate18PilotCompany {
  ...
  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_BROOKFIELD_NOT_UNIQUE",
    );
  }
...
function artifactReader(
  ...
  return (pin) => {
    ...
    if (!normalizedAbsolute.startsWith(normalizedPrefix)) {
      throw new Error(
        "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_PRIVATE_PATH_ESCAPE",
      );
    }
    ...
    try {
      ...
      throw new Error(
        `VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_PINNED_GIT_READ_FAILED:${message}`,
      );
    }
  };
}
...
function buildCompactPacket(
  ...
  for (const id of REQUIRED_EVIDENCE_IDS) {
    if (!evidenceItems.some((item) => item.evidence_id === id)) {
      throw new Error(
        `VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_EVIDENCE_MISSING:${id}`,
      );
    }
  }
...
function readAndAssertAuthorization(
  ...
  if (!options.execute) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_EXECUTE_FLAG_REQUIRED",
    );
  }

  if (options.authorizationId !== AUTHORIZATION_ID) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_AUTHORIZATION_ID_MISMATCH",
    );
  }
  ...
  if (!existsSync(absolute)) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_AUTHORIZATION_ARTIFACT_MISSING",
    );
  }
  ...
  if (
    ...
    artifact.phi4_mini_inference?.authorized !== true ||
    artifact.phi4_mini_inference.model_name !== MODEL_NAME ||
    artifact.phi4_mini_inference.model_digest !== MODEL_DIGEST ||
    artifact.phi4_mini_inference.semantic_probe_id !==
      GATE18_V10_BROOKFIELD_TARGETED_PROBE_ID ||
    artifact.phi4_mini_inference.execution_id !==
      "BROOKFIELD_PEER_ROLE_CORE_001_LOCAL_COMPACT_PHI4MINI_4096" ||
    artifact.phi4_mini_inference.context_tokens !== CONTEXT_TOKENS ||
    artifact.phi4_mini_inference.max_output_tokens !== MAX_OUTPUT_TOKENS ||
    artifact.phi4_mini_inference.temperature !== 0
  ) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_AUTHORIZATION_ARTIFACT_INVALID",
    );
  }
...
async function fetchJson<T>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        [
          `VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_OLLAMA_HTTP_${response.status}`,
...
function assertNoLoadedModels(): void {
  try {
    ...
    if (lines.length > 1) {
      throw new Error(
        "VNEXT_GATE18_PHASE_C_PHI4_MINI_OTHER_MODEL_ALREADY_LOADED",
      );
    }
  } catch (error) {
    if (
    ...
    throw new Error(
      "VNEXT_GATE18_PHASE_C_PHI4_MINI_OLLAMA_PS_CHECK_FAILED",
    );
  }
}
...
async function assertInstalledModel(): Promise<OllamaTag> {
  ...
  if (!model) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_MODEL_NOT_INSTALLED",
    );
  }

  if (model.digest !== MODEL_DIGEST) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_MODEL_DIGEST_MISMATCH",
    );
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (providerResponse.model !== MODEL_NAME) {
      throw new Error(
        "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_RESPONSE_MODEL_MISMATCH",
      );
    }

    if (providerResponse.done !== true) {
      throw new Error(
        "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_GENERATION_NOT_DONE",
      );
    }

    if (typeof providerResponse.response !== "string") {
      throw new Error(
        "VNEXT_GATE18_PHASE_C_PHI4_MINI_LOCAL_RESPONSE_TEXT_MISSING",
      );
    }
  ...
  const privateArtifact = {
    format:
      "OROTITAN_GATE18_PHASE_C_PHI4_MINI_PRIVATE_RUN_V0.1",
    privateArtifact: true,
    publication: false,
    productionMutation: false,
    gate: 18,
    phase: "C_LOCAL_FIRST_MODEL_QUALIFICATION",
    stage: "POST_C4_CANDIDATE_ESCALATION_SEMANTIC_PROBE",
    status,
    authorization: {
      id: AUTHORIZATION_ID,
  ...
  const outputPath = resolve(
    privateDir,
    `${isoFileSafe(completedAt)}__BROOKFIELD_PEER_ROLE_CORE_001_LOCAL_COMPACT_PHI4MINI_4096.json`,
  );
  ...
  console.log(
    JSON.stringify(
      {
        format:
          "OROTITAN_GATE18_PHASE_C_PHI4_MINI_EXECUTION_SUMMARY_V0.1",
        status,
        semanticProbeId:
          GATE18_V10_BROOKFIELD_TARGETED_PROBE_ID,
        phaseCExecutionId:
