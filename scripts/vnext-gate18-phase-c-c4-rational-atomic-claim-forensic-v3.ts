...
function parseArgs(argv: readonly string[]): {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    throw new Error(
      `VNEXT_GATE18_C4_RATIONAL_ATOMIC_CLAIM_FORENSIC_V3_UNKNOWN_ARG:${argv[i]}`,
    );
  }

  if (!privateRepoRoot.trim()) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_ATOMIC_CLAIM_FORENSIC_V3_PRIVATE_REPO_ROOT_REQUIRED",
    );
  }
  if (!privateArtifact.trim()) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_ATOMIC_CLAIM_FORENSIC_V3_PRIVATE_ARTIFACT_REQUIRED",
    );
  }
...
function findRational(): Gate18PilotCompany {
  ...
  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_ATOMIC_CLAIM_FORENSIC_V3_COMPANY_NOT_UNIQUE",
    );
  }
...
function artifactReader(
  ...
  return (pin) => {
    ...
    if (!normalizedAbsolute.startsWith(prefix)) {
      throw new Error(
        "VNEXT_GATE18_C4_RATIONAL_ATOMIC_CLAIM_FORENSIC_V3_PRIVATE_PATH_ESCAPE",
      );
    }
...
function main(): void {
  ...
  if (
    ...
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_ATOMIC_CLAIM_FORENSIC_V3_RUN_IDENTITY_MISMATCH",
    );
  }
  ...
  if (!parsed.success) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_ATOMIC_CLAIM_FORENSIC_V3_SCHEMA_REPARSE_FAILED",
    );
  }
  ...
  if (verified.packetSha256 !== EXPECTED_PACKET_SHA256) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_ATOMIC_CLAIM_FORENSIC_V3_PACKET_IDENTITY_MISMATCH",
    );
  }
  ...
  console.log(
    JSON.stringify(
      {
        format:
          "OROTITAN_GATE18_PHASE_C_C4_RATIONAL_ATOMIC_CLAIM_FORENSIC_V0.1",
        gate: 18,
