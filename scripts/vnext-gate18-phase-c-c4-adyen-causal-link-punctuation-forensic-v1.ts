...
function parseArgs(argv: readonly string[]): {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    throw new Error(
      `VNEXT_GATE18_C4_ADYEN_CAUSAL_LINK_FORENSIC_UNKNOWN_ARG:${argv[i]}`,
    );
  }

  if (!privateRepoRoot.trim()) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CAUSAL_LINK_FORENSIC_PRIVATE_REPO_ROOT_REQUIRED",
    );
  }
  if (!privateArtifact.trim()) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CAUSAL_LINK_FORENSIC_PRIVATE_ARTIFACT_REQUIRED",
    );
  }
...
function findAdyen(): Gate18PilotCompany {
  ...
  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CAUSAL_LINK_FORENSIC_COMPANY_NOT_UNIQUE",
    );
  }
...
function artifactReader(
  ...
  return (pin) => {
    ...
    if (!normalizedAbsolute.startsWith(prefix)) {
      throw new Error(
        "VNEXT_GATE18_C4_ADYEN_CAUSAL_LINK_FORENSIC_PRIVATE_PATH_ESCAPE",
      );
    }
...
function main(): void {
  ...
  if (
    ...
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CAUSAL_LINK_FORENSIC_RUN_IDENTITY_MISMATCH",
    );
  }
  ...
  if (!parsed.success) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CAUSAL_LINK_FORENSIC_SCHEMA_REPARSE_FAILED",
    );
  }
  ...
  if (verified.packetSha256 !== EXPECTED_PACKET_SHA256) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CAUSAL_LINK_FORENSIC_PACKET_IDENTITY_MISMATCH",
    );
  }

  const normalized = structuredClone(parsed.data);

  const incompleteCausalLinks = normalized.priority_findings
    .map((finding, index) => {
      const trimmed = finding.causal_link.trim();
      return {
        index: index + 1,
        length: trimmed.length,
        terminalPunctuationPresent: /[.!?]$/.test(trimmed),
      };
    })
    .filter((item) => !item.terminalPunctuationPresent);
  ...
  console.log(
    JSON.stringify(
      {
        format:
          "OROTITAN_GATE18_PHASE_C_C4_ADYEN_CAUSAL_LINK_PUNCTUATION_FORENSIC_V0.1",
        gate: 18,
        ...
        original: {
          incompleteCausalLinks,
          incompleteCausalLinkCount: incompleteCausalLinks.length,
        },
        diagnosticNormalization: {
          rule:
            "Append one period only to causal_link values lacking terminal . ! or ? punctuation.",
          normalizedFindingIndexes,
          rawCausalLinkTextIncludedInConsole: false,
        ...
        interpretationBoundary: {
          ...
          soleDefectConcluded:
            ...
            normalizedFindingIndexes.length > 0,
          modelCapabilityFailureConcluded: false,
          retryAuthorized: false,
          purpose:
            "Determine whether any downstream deterministic semantic defect exists after diagnostic punctuation normalization of incomplete causal links.",
        },
        safety: {
