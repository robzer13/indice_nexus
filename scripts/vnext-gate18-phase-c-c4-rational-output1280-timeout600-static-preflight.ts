...
function parsePrivateRepoRoot(argv: readonly string[]): string {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    throw new Error(
      `VNEXT_GATE18_C4_RATIONAL_OUTPUT1280_PREFLIGHT_UNKNOWN_ARG:${argv[index]}`,
    );
  }

  if (!root.trim()) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_OUTPUT1280_PREFLIGHT_PRIVATE_REPO_ROOT_REQUIRED",
    );
  }
...
function artifactReader(
  ...
  return (pin) => {
    ...
    if (!normalizedAbsolute.startsWith(prefix)) {
      throw new Error(
        "VNEXT_GATE18_C4_RATIONAL_OUTPUT1280_PREFLIGHT_PRIVATE_PATH_ESCAPE",
      );
    }
...
function findRationalAg(): Gate18PilotCompany {
  ...
  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_OUTPUT1280_PREFLIGHT_COMPANY_NOT_UNIQUE",
    );
  }
...
function main(): void {
  ...
  if (verified.packetSha256 !== EXPECTED_PACKET_SHA256) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_OUTPUT1280_PREFLIGHT_PACKET_IDENTITY_MISMATCH",
    );
  }
  if (promptSha256 !== EXPECTED_PROMPT_SHA256) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_OUTPUT1280_PREFLIGHT_PROMPT_IDENTITY_MISMATCH",
    );
  }
  if (promptBytes !== EXPECTED_PROMPT_BYTES) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_OUTPUT1280_PREFLIGHT_PROMPT_BYTES_MISMATCH",
    );
  }
  if (
    ...
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_OUTPUT1280_PREFLIGHT_PACKET_COUNTS_MISMATCH",
    );
  }
  ...
  console.log(
    JSON.stringify(
      {
        format:
          "OROTITAN_GATE18_PHASE_C_C4_RATIONAL_OUTPUT1280_TIMEOUT600_STATIC_REQUEST_PREFLIGHT_V0.1",
        gate: 18,
        ...
        runtimeBasis: {
          ...
          rationale:
            "Measure the 1280-token request candidate after Brookfield and Adyen both required roughly 1100 output tokens; RATIONAL's much larger evidence packet means adequacy is explicitly not concluded by this static preflight.",
        },
        interpretationBoundary: {
          ...
          nextAction:
            "Review exact RATIONAL AG output1280 request identity and decide whether a separate output-budget analysis is required before preparing any inference runner or authorization.",
        },
        safety: {
