...
const AUTHORIZATION_ID =
  "G18-PHASEC-C4-STM-QWEN4B-CONTEXT16384-GUARDED-RETRY-AUTH-001";
const AUTHORIZATION_PATH =
  "calibration/vnext/OROTITAN_GATE18_PHASE_C_C4_STMICRO_QWEN3_4B_CONTEXT16384_GUARDED_RETRY_AUTH_001.json";
...
const ATTEMPT_ID =
  "C4_STMICRO_MOAT_EVIDENCE_AUDIT_QWEN4B_CONTEXT16384_GUARDED_RETRY_001";
...
function readAndAssertAuthorization(
  ...
  if (
    ...
    artifact.status !== "AUTHORIZED_SINGLE_LOCAL_GUARDED_RETRY" ||
...
async function fetchJson<T>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        [
          `VNEXT_GATE18_PHASE_C_C4_LOCAL_OLLAMA_HTTP_${response.status}`,
...
async function main(): Promise<void> {
  ...
  const privateArtifact = {
    format:
      "OROTITAN_GATE18_PHASE_C_C4_LOCAL_GUARDED_RETRY_PRIVATE_RUN_V0.1",
  ...
  const outputPath = resolve(
    privateDir,
    `${isoFileSafe(completedAt)}__C4_STMICRO_MOAT_EVIDENCE_AUDIT_QWEN4B_CONTEXT16384_001.json`,
  );
