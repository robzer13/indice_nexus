...
function assertPatternParitySelfCheck(): void {
  ...
  for (const { label, pattern } of NON_ATOMIC_PATTERNS) {
    ...
    if (!probe || !pattern.test(probe)) {
      throw new Error(
        `VNEXT_GATE18_C4_RATIONAL_COUNTEREVIDENCE_LINK_FORENSIC_V4_PATTERN_SELF_CHECK_FAILED:${label}`,
      );
    }
  }
}
...
function parseArgs(argv: readonly string[]): {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    throw new Error(
      `VNEXT_GATE18_C4_RATIONAL_COUNTEREVIDENCE_LINK_FORENSIC_V4_UNKNOWN_ARG:${argv[i]}`,
    );
  }

  if (!privateRepoRoot.trim()) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_COUNTEREVIDENCE_LINK_FORENSIC_V4_PRIVATE_REPO_ROOT_REQUIRED",
    );
  }
  if (!privateArtifact.trim()) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_COUNTEREVIDENCE_LINK_FORENSIC_V4_PRIVATE_ARTIFACT_REQUIRED",
    );
  }
...
function findRational(): Gate18PilotCompany {
  ...
  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_COUNTEREVIDENCE_LINK_FORENSIC_V4_COMPANY_NOT_UNIQUE",
    );
  }
...
function artifactReader(
  ...
  return (pin) => {
    ...
    if (!normalizedAbsolute.startsWith(prefix)) {
      throw new Error(
        "VNEXT_GATE18_C4_RATIONAL_COUNTEREVIDENCE_LINK_FORENSIC_V4_PRIVATE_PATH_ESCAPE",
      );
    }
...
function main(): void {
  ...
  if (
    ...
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_COUNTEREVIDENCE_LINK_FORENSIC_V4_RUN_IDENTITY_MISMATCH",
    );
  }
  ...
  if (!parsed.success) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_COUNTEREVIDENCE_LINK_FORENSIC_V4_SCHEMA_REPARSE_FAILED",
    );
  }
  ...
  if (verified.packetSha256 !== EXPECTED_PACKET_SHA256) {
    throw new Error(
      "VNEXT_GATE18_C4_RATIONAL_COUNTEREVIDENCE_LINK_FORENSIC_V4_PACKET_IDENTITY_MISMATCH",
    );
  }
  ...
  const incompleteCounterevidenceLinks: Array<{
    findingIndex: number;
  ...
  normalized.priority_findings.forEach((finding, findingIndex) => {
    ...
    const trimmed = finding.counterevidence_link.trim();
    const reasons: string[] = [];

    if (!/[.!?]$/.test(trimmed)) {
      reasons.push("MISSING_TERMINAL_PUNCTUATION");
    }
    if (trimmed.length >= SATURATION_BOUNDARY) {
      reasons.push("NARRATIVE_BOUNDARY_SATURATION");
    }

    if (reasons.length > 0) {
      incompleteCounterevidenceLinks.push({
        findingIndex: findingIndex + 1,
  ...
  console.log(
    JSON.stringify(
      {
        format:
          "OROTITAN_GATE18_PHASE_C_C4_RATIONAL_COUNTEREVIDENCE_LINK_FORENSIC_V4_V0.1",
        gate: 18,
        ...
        diagnosticNormalization: {
          ...
          counterevidenceLinkRule:
            "Replace only non-null counterevidence_link values that fail the frozen complete-narrative rule (terminal punctuation and trimmed length below 178) with a short diagnostic placeholder on the in-memory copy.",
          saturationBoundary: SATURATION_BOUNDARY,
          incompleteCounterevidenceLinks,
          incompleteCounterevidenceLinkCount:
            incompleteCounterevidenceLinks.length,
          counterevidenceLinkReplacement:
            "Diagnostic counterevidence link.",
          rawCounterevidenceTextIncludedInConsole: false,
        ...
        interpretationBoundary: {
          ...
          purpose:
            "Reveal whether any downstream deterministic semantic defect remains after cumulative diagnostic normalization of finding-conflict grounding, direction-role overlap, non-atomic contrastive claims, and incomplete counterevidence links.",
        },
        safety: {
