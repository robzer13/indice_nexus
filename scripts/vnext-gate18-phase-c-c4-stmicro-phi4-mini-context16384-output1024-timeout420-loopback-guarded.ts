...
const AUTHORIZATION_ID =
  "G18-PHASEC-C4-STM-PHI4MINI-CONTEXT16384-OUTPUT1024-TIMEOUT420-LOOPBACK-AUTH-001";
...
const MATRIX_CELL_ID =
  "C4_STMICRO_MOAT_EVIDENCE_AUDIT_PHI4MINI_CONTEXT16384_001";
const ATTEMPT_ID =
  "C4_STMICRO_MOAT_EVIDENCE_AUDIT_PHI4MINI_CONTEXT16384_OUTPUT1024_TIMEOUT420_LOOPBACK_GUARDED_001";
const EXPECTED_PROMPT_SHA256 =
  "3f06d6e22dbbc88abbbc64fd470811f438a595c71154f7fcb8c004a6d4f5bc91";
const EXPECTED_PROMPT_BYTES = 7558;
const EXPECTED_EVIDENCE_COUNT = 9;
const EXPECTED_CONFLICT_COUNT = 2;
...
function findSTMicroelectronics(): Gate18PilotCompany {
  const matches = pilotJson.companies.filter(
    (company) =>
      company.display_name === "STMicroelectronics",
  );

  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_C4_LOCAL_STMICRO_NOT_UNIQUE",
    );
  }
...
function readAndAssertAuthorization(
  ...
  if (
    ...
    artifact.c4_inference.company !== "STMicroelectronics" ||
...
function assertNoLoadedModels(): void {
  try {
    ...
    if (lines.length > 1) {
      throw new Error(
        "VNEXT_GATE18_PHASE_C_C4_PHI4MINI_OTHER_MODEL_ALREADY_LOADED",
      );
    }
  } catch (error) {
    if (
    ...
    throw new Error(
      "VNEXT_GATE18_PHASE_C_C4_PHI4MINI_OLLAMA_PS_CHECK_FAILED",
    );
  }
}
...
async function main(): Promise<void> {
  ...
  const privateArtifact = {
    format:
      "OROTITAN_GATE18_PHASE_C_C4_PHI4_MINI_LOCAL_PRIVATE_RUN_V0.1",
  ...
  const outputPath = resolve(
    privateDir,
    `${isoFileSafe(completedAt)}__C4_STMICRO_MOAT_EVIDENCE_AUDIT_PHI4MINI_CONTEXT16384_001.json`,
  );
  ...
  console.log(
    JSON.stringify(
      {
        format:
          "OROTITAN_GATE18_PHASE_C_C4_PHI4_MINI_EXECUTION_SUMMARY_V0.1",
        status,
        matrixCellId: MATRIX_CELL_ID,
        attemptId: ATTEMPT_ID,
        company: "STMicroelectronics",
