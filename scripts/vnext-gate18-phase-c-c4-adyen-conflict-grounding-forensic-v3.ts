...
function parseArgs(argv: readonly string[]): {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    throw new Error(
      `VNEXT_GATE18_C4_ADYEN_CONFLICT_GROUNDING_FORENSIC_V3_UNKNOWN_ARG:${argv[i]}`,
    );
  }

  if (!privateRepoRoot.trim()) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CONFLICT_GROUNDING_FORENSIC_V3_PRIVATE_REPO_ROOT_REQUIRED",
    );
  }
  if (!privateArtifact.trim()) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CONFLICT_GROUNDING_FORENSIC_V3_PRIVATE_ARTIFACT_REQUIRED",
    );
  }
...
function findAdyen(): Gate18PilotCompany {
  ...
  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CONFLICT_GROUNDING_FORENSIC_V3_COMPANY_NOT_UNIQUE",
    );
  }
...
function artifactReader(
  ...
  return (pin) => {
    ...
    if (!normalizedAbsolute.startsWith(prefix)) {
      throw new Error(
        "VNEXT_GATE18_C4_ADYEN_CONFLICT_GROUNDING_FORENSIC_V3_PRIVATE_PATH_ESCAPE",
      );
    }
...
function main(): void {
  ...
  if (
    ...
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CONFLICT_GROUNDING_FORENSIC_V3_RUN_IDENTITY_MISMATCH",
    );
  }
  ...
  if (!parsed.success) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CONFLICT_GROUNDING_FORENSIC_V3_SCHEMA_REPARSE_FAILED",
    );
  }
  ...
  if (verified.packetSha256 !== EXPECTED_PACKET_SHA256) {
    throw new Error(
      "VNEXT_GATE18_C4_ADYEN_CONFLICT_GROUNDING_FORENSIC_V3_PACKET_IDENTITY_MISMATCH",
    );
  }
  ...
  const punctuationNormalizedFindingIndexes: number[] = [];

  normalized.priority_findings.forEach((finding, index) => {
    const trimmed = finding.causal_link.trim();
    if (!/[.!?]$/.test(trimmed)) {
      finding.causal_link = `${trimmed}.`;
      punctuationNormalizedFindingIndexes.push(index + 1);
    }
  });

  const saturatedNarratives: NarrativeRecord[] = [];

  const replaceIfSaturated = (
    path: string,
    kind: string,
    value: string,
    replacement: string,
    setter: (replacementValue: string) => void,
  ) => {
    const length = value.trim().length;
    if (length >= SATURATION_BOUNDARY) {
      saturatedNarratives.push({ path, length, kind });
      setter(replacement);
    }
  };
  ...
  normalized.priority_findings.forEach((finding, findingIndex) => {
    replaceIfSaturated(
      `priority_findings[${findingIndex}].claim`,
      "claim",
      finding.claim,
      "Diagnostic atomic claim.",
      (value) => {
        finding.claim = value;
      },
    );
    replaceIfSaturated(
      `priority_findings[${findingIndex}].causal_link`,
      "causal_link",
      finding.causal_link,
      "Diagnostic causal link.",
      (value) => {
    ...
    finding.evidence_qualifications.forEach(
      (qualification, qualificationIndex) => {
        replaceIfSaturated(
          ...
          "Diagnostic evidence qualification.",
          (value) => {
    ...
    if (finding.counterevidence_link !== null) {
      replaceIfSaturated(
        `priority_findings[${findingIndex}].counterevidence_link`,
        "counterevidence_link",
        finding.counterevidence_link,
        "Diagnostic counterevidence link.",
        (value) => {
  ...
  normalized.material_conflicts.forEach((conflict, index) => {
    replaceIfSaturated(
      `material_conflicts[${index}].implication`,
      "conflict_implication",
      conflict.implication,
      "Diagnostic conflict implication.",
      (value) => {
  ...
  normalized.weak_link_candidates.forEach((candidate, index) => {
    replaceIfSaturated(
      `weak_link_candidates[${index}].why_uncertain`,
      "weak_link_explanation",
      candidate.why_uncertain,
      "Diagnostic weak-link explanation.",
      (value) => {
  ...
  normalized.unresolved_points.forEach((point, index) => {
    replaceIfSaturated(
      `unresolved_points[${index}].question`,
      "unresolved_question",
      point.question,
      "Diagnostic unresolved question?",
      (value) => {
  ...
  console.log(
    JSON.stringify(
      {
        format:
          "OROTITAN_GATE18_PHASE_C_C4_ADYEN_CONFLICT_GROUNDING_FORENSIC_V0.1",
        gate: 18,
        ...
        diagnosticNormalization: {
          punctuationNormalizedFindingIndexes,
          saturationBoundary: SATURATION_BOUNDARY,
          saturatedNarratives,
          saturatedNarrativeCount: saturatedNarratives.length,
          ...
          replacementPolicy:
            "Replace only narratives at or above the frozen saturation boundary with short field-appropriate diagnostic placeholders.",
        ...
        interpretationBoundary: {
          ...
          purpose:
            "Reveal whether any downstream deterministic semantic defect remains after cumulative diagnostic normalization of causal-link completeness, narrative-boundary saturation, and finding-conflict grounding.",
        },
        safety: {
