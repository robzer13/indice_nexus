...
const AUTHORIZATION_ID =
  "G18-PHASEC-C4-ADYEN-QWEN4B-CONTEXT16384-OUTPUT1280-TIMEOUT600-LOOPBACK-AUTH-001";
const AUTHORIZATION_PATH =
  "calibration/vnext/OROTITAN_GATE18_PHASE_C_C4_ADYEN_QWEN3_4B_CONTEXT16384_OUTPUT1280_TIMEOUT600_LOOPBACK_AUTH_001.json";
...
function findAdyen(): Gate18PilotCompany {
  ...
  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_C4_LOCAL_ADYEN_NOT_UNIQUE",
    );
  }
...
function readAndAssertAuthorization(
  ...
  if (
    ...
    artifact.c4_inference.company !== "Adyen" ||
...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(
    privateDir,
    `${isoFileSafe(completedAt)}__C4_ADYEN_MOAT_EVIDENCE_AUDIT_QWEN4B_CONTEXT16384_001.json`,
  );
