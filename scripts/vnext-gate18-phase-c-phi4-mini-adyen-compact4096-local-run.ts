...
const AUTHORIZATION_ID =
  "G18-PHASEC-PHI4-MINI-ADYEN-COMPACT4096-INFERENCE-AUTH-001";
...
function findAdyen(): Gate18PilotCompany {
  ...
  if (matches.length !== 1) {
    throw new Error(
      "VNEXT_GATE18_PHASE_C_C3_LOCAL_ADYEN_NOT_UNIQUE",
    );
  }
...
function buildCompactPacket(
  ...
  for (const id of REQUIRED_CONFLICT_IDS) {
    if (!conflicts.some((item) => item.conflict_id === id)) {
      throw new Error(
        `VNEXT_GATE18_PHASE_C_C3_LOCAL_CONFLICT_MISSING:${id}`,
      );
    }
  }
...
function assertNoLoadedModels(): void {
  try {
    ...
    if (lines.length > 1) {
      throw new Error(
        "VNEXT_GATE18_PHASE_C_C3_QWEN4B_OTHER_MODEL_ALREADY_LOADED",
      );
    }
  } catch (error) {
    if (
    ...
    throw new Error(
      "VNEXT_GATE18_PHASE_C_C3_QWEN4B_OLLAMA_PS_CHECK_FAILED",
    );
  }
}
...
async function main(): Promise<void> {
  ...
  const modelInput =
    buildGate18V10TargetedProbeInput(compactPacket);
  ...
  const privateArtifact = {
    ...
    expectedSemantics: {
      ...
      finding1: {
        supportState: "MIXED",
        evidenceIds: ["E-036", "E-042"],
        conflictIds: ["C-005"],
        counterevidenceIds: ["E-037", "E-041"],
      },
      finding2: {
      ...
      finding3: {
        supportState: "SUPPORTED",
        evidenceIds: ["E-040", "E-043"],
        conflictIds: [],
        counterevidenceIds: [],
        qualificationIds: ["E-040", "E-043"],
      },
      conflictStates: {
  ...
  const outputPath = resolve(
    privateDir,
    `${isoFileSafe(completedAt)}__ADYEN_CLAIM_TARGET_CORE_001_LOCAL_COMPACT_PHI4MINI_4096.json`,
  );
