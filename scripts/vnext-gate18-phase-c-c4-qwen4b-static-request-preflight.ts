...
function parsePrivateRepoRoot(argv: readonly string[]): string {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    throw new Error(`VNEXT_GATE18_C4_PREFLIGHT_UNKNOWN_ARG:${argv[i]}`);
  }
  if (!root.trim()) {
    throw new Error("VNEXT_GATE18_C4_PREFLIGHT_PRIVATE_REPO_ROOT_REQUIRED");
  }
  return root;
}
...
function artifactReader(
  ...
  return (pin) => {
    ...
    if (!normalizedAbsolute.startsWith(prefix)) {
      throw new Error("VNEXT_GATE18_C4_PREFLIGHT_PRIVATE_PATH_ESCAPE");
    }
...
function main(): void {
  ...
  console.log(JSON.stringify({
    format: "OROTITAN_GATE18_PHASE_C_C4_STATIC_REQUEST_PREFLIGHT_V0.1",
    gate: 18,
    ...
    interpretationBoundary: {
      ...
      nextAction: "Human review of exact five-company request sizes and hashes before any C4 execution authorization.",
    },
    safety: {
