...
const AUTHORIZATION_ID =
  "G18-PHASEC-C3-ADYEN-QWEN4B-INFERENCE-AUTH-002";
const AUTHORIZATION_PATH =
  "calibration/vnext/OROTITAN_GATE18_PHASE_C_C3_ADYEN_QWEN3_4B_INFERENCE_AUTH_002.json";
...
function readAndAssertAuthorization(
  ...
  if (
    ...
    artifact.c3_inference.execution_id !==
      "ADYEN_CLAIM_TARGET_CORE_001_LOCAL_COMPACT_QWEN4B_4096_TIMEOUT300" ||
    artifact.c3_inference.context_tokens !== CONTEXT_TOKENS ||
    artifact.c3_inference.max_output_tokens !== MAX_OUTPUT_TOKENS ||
    artifact.c3_inference.temperature !== 0 ||
    artifact.c3_inference.client_timeout_ms !== CLIENT_TIMEOUT_MS
  ) {
    throw new Error(
...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(
    privateDir,
    `${isoFileSafe(completedAt)}__ADYEN_CLAIM_TARGET_CORE_001_LOCAL_COMPACT_QWEN4B_4096_TIMEOUT300.json`,
  );
