...
export default async function CompanyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="space-y-8">
    <section className="flex flex-col gap-5 border-b border-slate-800 pb-7 lg:flex-row lg:items-end lg:justify-between"><div><div className="font-mono text-xs text-cyan-400">{company.ticker} · {company.exchange}</div><h1 className="mt-2 text-3xl font-semibold text-white sm:text-4xl">{company.name}</h1><p className="mt-2 text-sm text-slate-500">{company.country ?? 'Pays non renseigné'} · {company.sector ?? 'Secteur non renseigné'}</p></div><div className="flex flex-wrap gap-2"><EntryZoneBadge zone={zone}/><CompanyStatusBadge status={company.status}/></div></section>
    ...
    <section className="space-y-4"><div><h2 className="text-xl font-semibold text-white">Historique de marché</h2><p className="mt-1 text-sm text-slate-500">Les cours proviennent uniquement de market_prices. Les seuils affichés appartiennent au snapshot courant.</p></div><PriceHistoryChart points={priceHistory} thresholds={thresholds.map(([label, value]) => ({ label, value }))} {...priceProps}/></section>

    <section className="space-y-4"><div><h2 className="text-xl font-semibold text-white">Valorisation</h2><p className="mt-1 text-sm text-slate-500">Fair values et seuils proviennent exclusivement du snapshot courant sélectionné.</p></div><FairValueRange company={company}/><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{thresholds.map(([label, value]) => <div key={label} className={`rounded-lg border p-4 ${label === 'O90' ? 'border-cyan-800 bg-cyan-950/20' : 'border-slate-800 bg-slate-900/50'}`}><div className="text-xs font-semibold text-slate-500">{label}</div><div className="mt-2 text-lg font-semibold text-slate-100">{value === null ? <span className="text-sm font-medium text-slate-500">Non calibré</span> : <PriceDisplay value={value} {...priceProps}/>}</div></div>)}</div></section>
